...
class Command(BaseCommand):
    ...
    def create_profiles():
        users = [User(username=f"User {i}", password=random.randint(1000, 10000), email=f"email{i}@vk.ru") for i in range(1, 10000)]
        User.objects.bulk_create(users)
        users1 = User.objects.all()
        profiles = [Profile(user = users1[i]) for i in range(10000)]
        Profile.objects.bulk_create(profiles)

    def create_tags():
        tags = [Tag(name = f"Tag{i}") for i in range(1, 10000)]
    ...
    def create_questions():
        ...
        questions = [Question(head=f"Question number {i}", body = qbody, author = random.choice(profiles)) for i in range(50000, 100001)]
    ...
    def create_answers():
        ...
        answers = [Answer(question = random.choice(questions), body = f"Answer {i} " + abody, author = random.choice(profiles)) for i in range(960000, 1000001)]
    ...
    def create_qlikes():
        ...
        qlikes = [Like(content_type = qtype, vote = 1, user = random.choice(profiles), object_id = random.choice(questions_ids)) for i in range(15000)]
    ...
    def create_alikes():
        ...
        alikes = [Like(content_type = atype, vote = 1, user = random.choice(profiles), object_id = random.choice(answers_ids)) for i in range(15000)]
